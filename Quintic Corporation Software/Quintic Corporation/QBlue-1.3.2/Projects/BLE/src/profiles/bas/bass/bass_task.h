...
struct bass_enable_req
{
    ...
    struct prf_char_pres_fmt batt_level_pres_format[BASS_NB_BAS_INSTANCES_MAX];
	  ...
	  //ADDED BY JOE
	  /// Temperature Level Notification Configurations
    uint16_t temp_level_ntf_cfg[BASS_NB_BAS_INSTANCES_MAX];
    /// Last Temperature Level
    uint8_t old_temp_lvl[BASS_NB_BAS_INSTANCES_MAX];
    /// Current Temperature Level
    uint8_t current_temp_lvl[BASS_NB_BAS_INSTANCES_MAX];
    /// Temperature Level Characteristic Presentation Format - Should not change during connection
    struct prf_char_pres_fmt temp_level_pres_format[BASS_NB_BAS_INSTANCES_MAX];
...
struct bass_disable_ind
{
    ...
    uint8_t batt_lvl[BASS_NB_BAS_INSTANCES_MAX];
	  ...
	  //ADDED BY JOE
	  /// Battery Level
    uint8_t temp_lvl[BASS_NB_BAS_INSTANCES_MAX];
};
