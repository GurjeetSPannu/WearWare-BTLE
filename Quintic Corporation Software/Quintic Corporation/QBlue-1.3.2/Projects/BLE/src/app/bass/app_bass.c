...
void app_bass_enable_req(uint16_t conhdl, uint8_t bas_nb, uint8_t sec_lvl, uint8_t con_type, 
                         ...
                         uint8_t *current_batt_lvl, struct prf_char_pres_fmt *batt_level_pres_format
												 //ADDED BY JOE
												 ,  uint8_t *old_temp_lvl,
                         uint8_t *current_temp_lvl, struct prf_char_pres_fmt *temp_level_pres_format)
{
    ...
    for (uint8_t i = 0; i < bas_nb; i++)
    {
        ...
        msg->batt_level_pres_format[i] = batt_level_pres_format[i];
			  ...
			  //ADDED BY JOE
			  msg->old_temp_lvl[i] = old_temp_lvl[i];
        msg->current_temp_lvl[i] = current_temp_lvl[i];
        msg->temp_level_pres_format[i] = temp_level_pres_format[i];
    }
    ke_msg_send(msg);
}
 ...
 ****************************************************************************************
 */
void app_bass_batt_level_upd_req(uint16_t conhdl, uint8_t bas_instance, uint8_t batt_level, uint8_t temp_level)
{
