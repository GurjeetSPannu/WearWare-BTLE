                                ...
                                ke_task_id_t const src_id)
{
    ...
    if (param->status == CO_ERROR_NO_ERROR) 
    {
        ...
        app_qppc_env->cur_code = QPPC_QPPS_RX_CHAR_VALUE_USER_DESP;
        app_qppc_env->nb_ntf_char = param->nb_ntf_char;
        app_qppc_rd_char_req(param->conhdl, QPPC_QPPS_RX_CHAR_VALUE_USER_DESP);
    }
                                 ...
                                 ke_task_id_t const src_id)
{
    ...
    if (app_qppc_env->cur_code == QPPC_QPPS_RX_CHAR_VALUE_USER_DESP)
    {
        app_qppc_cfg_indntf_req(PRF_CLI_START_NTF, param->conhdl, QPPC_QPPS_FIRST_TX_VALUE_CLI_CFG);
        app_qppc_env->cur_code = QPPC_QPPS_FIRST_TX_VALUE_CLI_CFG;
    }
    app_task_msg_hdl(msgid, param);
                                 ...
                                 ke_task_id_t const src_id)
{
    ...
    if (param->status == CO_ERROR_NO_ERROR)
    {
        if (app_qppc_env->cur_code < app_qppc_env->nb_ntf_char)
        {
            app_qppc_cfg_indntf_req(PRF_CLI_START_NTF, param->conhdl, ++app_qppc_env->cur_code);
        }
    }
                              ...
                              ke_task_id_t const src_id)
{
    if (param->length > 0)
    {
        QPRINTF("Received %d bytes data[%02X] from characteristic %d\r\n", param->length, param->data[0], param->char_code);
    }
    app_task_msg_hdl(msgid, param);
...
int app_qppc_disable_ind_handler(ke_msg_id_t const msgid,
                                 ...
                                 ke_task_id_t const src_id)
{
    app_qppc_env->conhdl = 0xFFFF;
    app_qppc_env->enabled = false;
    app_qppc_env->nb_ntf_char = 0;
