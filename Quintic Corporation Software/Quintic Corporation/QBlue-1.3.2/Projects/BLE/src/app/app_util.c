    ...
    if (enabled == true)
    {
        ...
        app_bass_enable_req(conhdl, app_bass_env->bas_nb, PERM_RIGHT_ENABLE, PRF_CON_NORMAL, 
                            ...
                            app_bass_env->current_batt_lvl, app_bass_env->batt_level_pres_format,
														//ADDED BY JOE
														app_bass_env->old_temp_lvl, 
                            app_bass_env->current_temp_lvl, app_bass_env->temp_level_pres_format);
...
#if ( BLE_AN_SERVER || BLE_CSC_SENSOR || BLE_OTA_SERVER || BLE_PAS_SERVER || BLE_HT_THERMOM || BLE_RSC_SENSOR || BLE_BP_SENSOR || BLE_HR_SENSOR || BLE_GL_SENSOR || BLE_TIP_SERVER || BLE_SP_SERVER || BLE_HID_DEVICE || BLE_PROX_REPORTER || BLE_FINDME_TARGET || BLE_DIS_SERVER || BLE_BATT_SERVER)
