...
/**
 ****************************************************************************************
 * @brief Handles WW protocol Sleep timer
 *
 * @param[in] msgid     Id of the message received
 * @param[in] param     None
 * @param[in] dest_id   TASK_APP
 ...
 ****************************************************************************************
 */
int app_protocol_sleep_timer_handler(ke_msg_id_t const msgid, void const *param,
                               ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    switch(msgid)
    {
			case APP_SYS_SLEEP_TIMER:
						send_packet(PROTOCOL_MODE_TX2);
						//QPRINTF("Packet Sent\r\n");
...
void usr_init(void)
{
                                            ...
                                            app_event_accel_handler))
    {
		...
		//PROTOCOL
		//testing protocol
		//add_to_list(PROTOCOL_BATT,true);
		//add_to_list(PROTOCOL_TEMP,true);
		char *data;
		data = malloc(sizeof(char)*12);
		data[0] = PROTOCOL_COMMAND_ENABLE;
		data[1] = PROTOCOL_BATT;
		
		data[2] = PROTOCOL_COMMAND_ENABLE;
		data[3] = PROTOCOL_TEMP;
		
		data[4] = PROTOCOL_COMMAND_ENABLE;
		data[5] = PROTOCOL_PED;
		
		data[6] = PROTOCOL_COMMAND_RATE;
		data[7] = 0x00;
		data[8] = 0x00;   //10 int
		data[9] = 0x00;
		data[10] = 0x0A;
